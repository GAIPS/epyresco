...
def locally(combos, cpus):
    configs = [
        "python main.py "
        + " ".join([c for c in combo if c.startswith("--")])
        + " with "
        + " ".join([c for c in combo if not c.startswith("--")])
        for combo in combos
    ]
...
def single(combos, index):
    ...
    cmd = (
        "python main.py "
        + " ".join([c for c in config if c.startswith("--")])
        + " with "
        + " ".join([c for c in config if not c.startswith("--")])
    )
    print(cmd)
    work(cmd)
